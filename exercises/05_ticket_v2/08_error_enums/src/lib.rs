...
enum TicketNewError {
    TitleIsEmptyError,
    TitleIsTooLongError,
    DescriptionIsEmptyError,
    DescriptionIsTooLongError,
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(e) if e == TicketNewError::DescriptionIsEmptyError || e == TicketNewError::DescriptionIsTooLongError => {
            Ticket::new(title, "Description not provided".into(), status).unwrap()
        },
        Err(e) if e == TicketNewError::TitleIsEmptyError => panic!("Title cannot be empty"),
        Err(e) if e == TicketNewError::TitleIsTooLongError => panic!("Title cannot be longer than 50 bytes"),
        _ => unimplemented!()
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleIsEmptyError);
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleIsTooLongError);
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionIsEmptyError);
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionIsTooLongError);
        }
